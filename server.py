from flask import Flask, request, jsonify, send_file
from flask_cors import CORS   # ✅ ADD THIS
import backend
import analysis
import os

app = Flask(__name__)

# -------------------------------------------------
# ENABLE CORS FOR ALL ROUTES (FIXES FETCH ERRORS)
# -------------------------------------------------
CORS(app, resources={r"/*": {"origins": "*"}})   # ✅ FULL FIX



# ------------------ GLOBAL DATASET SELECTION ------------------ #

CURRENT_DATASET = backend.SELECTED_DATASET


def _get_dataset_id_from_request():
    ds = request.args.get("id")
    if ds:
        return ds
    return CURRENT_DATASET



@app.route("/", methods=["GET"])
def home():
    return """
    <h1>Backend Server Running ✔</h1>
    <p>Welcome to the hydrological Dashboard</p>
    """


# ------------------ SELECT DATASET LIKE OLD INPUT() ------------------ #

@app.route("/api/select", methods=["GET"])
def select_dataset():
    global CURRENT_DATASET
    ds_id = request.args.get("id")

    if ds_id not in backend.DATASETS:
        return jsonify({"error": "Invalid dataset id. Use 1, 2, or 3"}), 400

    CURRENT_DATASET = ds_id
    backend.SELECTED_DATASET = ds_id

    return jsonify({"selected": CURRENT_DATASET})


@app.route("/api/selected", methods=["GET"])
def get_selected_dataset():
    return jsonify({"selected_dataset": CURRENT_DATASET})



# ------------------ DATASET META / SUMMARY ------------------ #

@app.route("/api/datasets", methods=["GET"])
def list_datasets():
    return jsonify(backend.get_dataset_metadata())


@app.route("/api/summary", methods=["GET"])
def dataset_summary():
    ds_id = _get_dataset_id_from_request()
    summary = backend.get_basic_summary(ds_id)
    return jsonify(summary)



# ------------------ PLOTS ------------------ #

@app.route("/api/plot/correlation", methods=["GET"])
def plot_correlation():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_correlation_heatmap(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/ndci", methods=["GET"])
def plot_ndci():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_ndci_trend(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/turbidity", methods=["GET"])
def plot_turbidity():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_turbidity_trend(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/shrinkage", methods=["GET"])
def plot_shrinkage():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_shrinkage_trend(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/violin/ndci", methods=["GET"])
def plot_violin_ndci():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_violin_ndci(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/violin/turbidity", methods=["GET"])
def plot_violin_turbidity():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_violin_turbidity(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/violin/shrinkage", methods=["GET"])
def plot_violin_shrinkage():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_violin_shrinkage(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/box/ndci", methods=["GET"])
def plot_box_ndci():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_box_ndci(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/box/turbidity", methods=["GET"])
def plot_box_turbidity():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_box_turbidity(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/box/shrinkage", methods=["GET"])
def plot_box_shrinkage():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_box_shrinkage(ds_id)
    return send_file(img, mimetype="image/png")


@app.route("/api/plot/model_accuracy", methods=["GET"])
def plot_model_accuracy():
    ds_id = _get_dataset_id_from_request()
    img = backend.plot_model_accuracy(ds_id)
    return send_file(img, mimetype="image/png")



# ------------------ MODEL METRICS ------------------ #

@app.route("/api/model/metrics", methods=["GET"])
def model_metrics():
    ds_id = _get_dataset_id_from_request()
    metrics = backend.train_and_evaluate_model(ds_id)
    return jsonify(metrics)



# ------------------ GEMINI ANALYSIS ------------------ #

@app.route("/api/analyze", methods=["POST"])
def analyze():
    body = request.get_json(force=True) or {}
    ds_id = body.get("id", CURRENT_DATASET)
    user_prompt = body.get("prompt", "")
    result_text = analysis.generate_textual_analysis(ds_id, user_prompt)
    return jsonify({"analysis": result_text})


# ------------------ EXPORT PDF ENDPOINT ------------------ #


@app.route("/api/export/pdf", methods=["GET"])
def export_pdf():
    # get dataset id from query OR use current dataset
    ds_id = request.args.get("id", CURRENT_DATASET)

    # You can customize naming here
    filename_map = {
        "1": "analysis.pdf",
        "2": "analysis_2.pdf",
        "3": "analysis_3.pdf"
    }

    # default to analysis_1.pdf if ID not valid
    pdf_filename = filename_map.get(ds_id, "analysis.pdf")

    # simple relative path
    pdf_path = f"others/{pdf_filename}"

    if not os.path.exists(pdf_path):
        return jsonify({"error": f"PDF '{pdf_filename}' not found"}), 404

    return send_file(
        pdf_path,
        mimetype="application/pdf",
        as_attachment=True
    )


if __name__ == "__main__":
    app.run(host="0.0.0.0", port=8040, debug=True)
